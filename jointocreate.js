...
//voice state update event to check joining/leaving channels
  ...
  if (!oldState.channelID && newState.channelID) {
    if((newState.channelID !== config.JOINTOCREATECHANNEL) && (newState.channelID !== config.JOINTOCREATECHANNEL2)) return;  //if its not the jointocreatechannel skip
    if (newState.channelID == config.JOINTOCREATECHANNEL) {type = 1} else if (newState.channelID == config.JOINTOCREATECHANNEL2) {type = 2}
	 jointocreatechannel(newState, type);   //load the function
  }
  // LEFT V12
  if (oldState.channelID && !newState.channelID) {
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    if (oldState.channelID !== newState.channelID) {
      ...
      if((newState.channelID===config.JOINTOCREATECHANNEL) || (newState.channelID===config.JOINTOCREATECHANNEL2)) {
			//make a new channel
			if (newState.channelID == config.JOINTOCREATECHANNEL) {type = 1} else if (newState.channelID == config.JOINTOCREATECHANNEL2) {type = 2}
			jointocreatechannel(newState, type);
		}
    ...
    async function jointocreatechannel(user, type) {
      //log it 
      ...
      //user.member.user.send("This can be used to message the member that a new room was created")
		let channelParent;
		if (type == 1) {channelParent = config.CATEGORY1} else if (type == 2) {channelParent = config.CATEGORY2}
      await user.guild.channels.create(`${user.member.nickname}'s Channel`, { //user.member.user.username
        type: 'voice',
        parent: channelParent, //or set it as a category id. user.channel.parent.id
      }).then(async vc => {
